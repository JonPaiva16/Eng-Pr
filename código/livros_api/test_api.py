...
def test_buscar_livro_por_titulo():
    _criar_livro_exemplo(titulo="Harry Potter")
    _criar_livro_exemplo(titulo="Senhor dos Aneis")

    resp = client.get("/livros/busca?titulo=Harry")

    assert resp.status_code == 200

    livros = resp.json()
    assert len(livros) >= 1
    assert "Harry" in livros[0]["titulo"]


def test_busca_por_titulo_inexistente_retorna_lista_vazia():
    resp = client.get("/livros/busca?titulo=LivroQueNaoExiste")
